...
// Pinclaw command handler (ABD characteristic)
static ssize_t text_write_handler(struct bt_conn *conn,
                                  const struct bt_gatt_attr *attr,
...
static struct bt_uuid_128 audio_service_uuid =
    BT_UUID_INIT_128(BT_UUID_128_ENCODE(0x12345678, 0x1234, 0x1234, 0x1234, 0x123456789ABC));
static struct bt_uuid_128 audio_characteristic_data_uuid =
    BT_UUID_INIT_128(BT_UUID_128_ENCODE(0x12345678, 0x1234, 0x1234, 0x1234, 0x123456789ABE));
static struct bt_uuid_128 audio_characteristic_format_uuid =
    BT_UUID_INIT_128(BT_UUID_128_ENCODE(0x12345678, 0x1234, 0x1234, 0x1234, 0x123456789ABD));
...
struct bt_gatt_service audio_service = BT_GATT_SERVICE(audio_service_attr);
...
static bool read_from_tx_queue()
{
    ...
    if (tx_buffer_size != (CODEC_OUTPUT_MAX_BYTES + RING_BUFFER_HEADER_SIZE)) {
        // Ring buffer empty — normal when not recording
        return false;
    }
...
static bool push_to_gatt(struct bt_conn *conn)
{
    ...
    // Send raw Pinclaw-formatted data (header already added in broadcast_audio_packets)
...
void pusher(void)
{
    ...
    while (1) {
        ...
        if (current_mtu < MINIMAL_PACKET_SIZE) {
            valid = false;
            reject_reason = "MTU too low";
        } else if (!conn) {
            valid = false;
            reject_reason = "no connection";
        } else {
            valid = bt_gatt_is_subscribed(conn, &audio_service.attrs[1], BT_GATT_CCC_NOTIFY);
            if (!valid) reject_reason = "not subscribed";
        }

        // Log rejection reason once per recording session
        if (!valid && recording_active) {
            static int64_t last_reject_log = 0;
            int64_t now = k_uptime_get();
            if (now - last_reject_log > 2000) {  // throttle: log every 2s max
                LOG_WRN("[AUDIO-TX] BLOCKED: %s (mtu=%d, conn=%p, recording=%d)",
                        reject_reason, current_mtu, (void *)conn, recording_active);
                last_reject_log = now;
            }
        }
        ...
        if (ring_buf_is_empty(&ring_buf)) {
            k_sleep(K_MSEC(10));  // Don't busy-wait when no audio data
        } else {
            k_yield();
        }
    }
...
int broadcast_audio_packets(uint8_t *buffer, size_t size)
{
    ...
    // Wrap in Pinclaw format: [0x02][seqNo:2B BE][opus data]
    uint8_t pkt[3 + 320];  // 3 header + max opus frame
    if (size > 320) size = 320;
    pkt[0] = 0x02;  // PACKET_DATA
    pkt[1] = (opus_seq_no >> 8) & 0xFF;
    pkt[2] = opus_seq_no & 0xFF;
    memcpy(pkt + 3, buffer, size);
    opus_seq_no++;

    int retry_count = 0;
    const int max_retries = 3;

    while (retry_count < max_retries && !write_to_tx_queue(pkt, 3 + size)) {
        k_sleep(K_MSEC(1));
        retry_count++;
    }
...
// Enqueue END packet into ring buffer so pusher thread sends it
// after all audio frames — no BLE TX buffer competition with pusher
int enqueue_end_packet(uint16_t total_frames)
{
    uint8_t end_pkt[5];
    end_pkt[0] = 0x03;  // PACKET_END
    end_pkt[1] = (total_frames >> 24) & 0xFF;
    end_pkt[2] = (total_frames >> 16) & 0xFF;
    end_pkt[3] = (total_frames >> 8) & 0xFF;
    end_pkt[4] = total_frames & 0xFF;

    int retry_count = 0;
    const int max_retries = 10;
    while (retry_count < max_retries && !write_to_tx_queue(end_pkt, sizeof(end_pkt))) {
        k_sleep(K_MSEC(10));
        retry_count++;
    }

    if (retry_count >= max_retries) {
        LOG_ERR("[END] Failed to enqueue END packet after %d retries", max_retries);
        return -1;
    }

    LOG_INF("[END] END packet enqueued (frames=%d)", total_frames);
    return 0;
}

// Pinclaw command handler — iPhone writes 0x01 (start) / 0x00 (stop) / 0x40 (shutdown)
static ssize_t text_write_handler(struct bt_conn *conn,
                                  const struct bt_gatt_attr *attr,
                                  const void *buf, uint16_t len,
                                  uint16_t offset, uint8_t flags)
