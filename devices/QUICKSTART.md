...
The trusted Host is the official Nexting App. It owns credentials,
authorization, policy, and Agent-specific mapping. The device owns physical
input and minimum display state. This public repository documents the device
side of that contract; it does not provide a kit for building another App.
...
unpublished App build or weaken BLE authorization. Build and validate the
device locally; connect it through the official Nexting App when your device
...
remote Agent interaction. With the public SDK and protocol/firmware checks, it
is a local device proof: useful for validating the open contract, but not a
claim that the public Nexting App already enrolls DIY hardware.
...
- The same device contract works across richer custom hardware without
  exposing Agent credentials or App internals to firmware.
