...
- Reframed the Quickstart around the device–App–Agent architecture and split
  ...
  tutorial.
- Removed custom Apple and Android Host SDKs from the public candidate. The
  official Nexting App is the supported Host; this package is device-side.
...
- Shared valid and hostile vectors plus JSON Schema definitions for all 15 new
  messages.
- Strict JavaScript and fixed-buffer C99 implementations of every new message.
- Device Info profile negotiation helpers and device-side profile gating.
...
- The Codex guide now documents the public protocol boundary and freezes the
  fail-closed one-time approval projection into `approval/1`.
...
- A reproducible firmware and protocol verification path for the Developer
  Reference boards.
...
- Website SDK pages have stable shareable routes and keep the official App
  boundary explicit.
...
- Profile `status/1`: an App→device downlink that replaces the full rendered state of up to eight anonymous agent-status slots (`idle`, `thinking`, `working`, `complete`, `needs_input`, `error`) with an optional 64-byte label, negotiated through the new optional `statusSlots` Device Info capability field and cleared on disconnect or reboot. Ships with shared `status-v1.json` vectors, a schema def, and JavaScript and C implementations. The Zephyr reference does not declare `statusSlots` until per-slot indicator rendering is implemented and board-verified.
...
- A fixed-buffer portable C99 device core for firmware authors.
...
- Optional, bounded Device Info identity, hardware-capability, Battery Service,
  and vendor-extension metadata shared by the device SDK and protocol reference.
...
- A deterministic allowlisted exporter into `Nexting-ai/nexting/devices`,
  SHA-256 manifest, and hostile-input tests.

### Security and correctness

- Official JavaScript and C decoders share 15 valid and 29 hostile vectors, covering every enum and exact summary boundaries while consistently rejecting duplicate known fields, decoded NUL, lone surrogates, non-canonical integers, excessive nesting, oversized frames, invalid UTF-8, and over-limit summaries.
...
- The reference protocol tests exercise encrypted-link assumptions, monotonic TTL,
  and fragmented notifications without interleaving frames.
...
- JavaScript, C sanitizer, firmware, exporter, and boundary gates pass at the
  2026-07-27 snapshot.
